 ...
 */
export function createGetViewBackend() {
    return {
        name: 'getview',
        available: () => true,
            ...
            const dispose = ctx.on('internal/get', (consumerCtx, name, _error, next) => {
                ...
                return new Proxy(value, {
                    get(target, prop, receiver) {
                        ...
                        if (prop !== method || typeof member !== 'function')
                            return member;
                        return (...args) => {
                            const event = createForgeEvent(point, { service, method, args });
                            return dispatchCall(hooks, point, consumerCtx, event, (a) => Reflect.apply(member, receiver, a), options.mutate);
                        };
                    },
                });
            });
