...
export function GitView(props: {
  ...
  /** Present when the forge event stream should drive the changed-files list. */
  ctx?: Context
}) {
  const { scope, onOpenFile, onOpenDiff, ctx } = props
  ...
  // Forge event stream (dsh-plugin-demo host relay): `sidebar/diff` carries
  // numstat entries { add, del, path }. Project them onto the native git
  // status list as unstaged worktree changes so the existing Git pane UI
  // renders the forge-provided "changed files" without a separate demo tab.
  useEffect(() => {
