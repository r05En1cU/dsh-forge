  ...
  points: [
    {
      ...
      mixin: defineMixin({
        ...
        target: { module: '@official/chat', versionRange: '^1.0.0', filePath: 'lib/util.js', functionQuery: { functionName: 'helper', kind: 'Sync' } },
        operation: 'before',
      }),
    },
    {
      ...
      mixin: defineMixin({
        ...
        target: { module: '@official/chat', versionRange: '^1.0.0', filePath: 'lib/util.js', functionQuery: { functionName: 'compute', kind: 'Sync' } },
        operation: 'around',
      }),
    },
    {
      ...
      mixin: defineMixin({
        ...
        target: { module: '@official/chat', versionRange: '^1.0.0', filePath: 'lib/util.js', functionQuery: { functionName: 'greet', kind: 'Sync' } },
        operation: 'replace',
      }),
    },
  ],
...
test('e2e: real fabric engine, before-operation point', async (t) => {
  ...
  const seen: unknown[] = []
  ctx.on('official-chat/helper/before', (e) => { seen.push(e.args[0], e.mixin); e.args[0] = (e.args[0] as string).toUpperCase() })
  assert.equal(util.helper('real'), '[util] REAL')
  assert.deepEqual(seen, ['real', 'official-chat/helper'])
})
...
test('e2e: event object honors the cross-backend contract', async (t) => {
  ...
  // plus the first-class mixin id on fabric-backed points
  assert.deepEqual(beforeKeys, ['args', 'mixin', 'point', 'result', 'self'])
  assert.equal(afterEvent?.point, 'official-chat/compute')
  assert.equal(afterEvent?.mixin, 'official-chat/compute')
  assert.equal(afterEvent?.result, 42)
})

test('e2e: buildPatchStubs and runtime registration agree on patch ids', async (t) => {
  const ctx = await setup(t)
  const fabric = ctx.get('fabric') as any
  for (const point of catalog.points) {
    assert.ok(fabric.bindings(point.mixin!.id).length > 0, `${point.id}: missing binding for ${point.mixin!.id}`)
  }
  // keep the hooks handle alive for documentation purposes; the test process
  // exits after the suite, and registerHooks intentionally has no unregister.
  void disposeHooks
})
