import type { InjectionPoint, Catalog } from './types.ts'
 ...
 */
export function defineInjectionPoint(point: InjectionPoint): Readonly<InjectionPoint> {
...
export function defineCatalog(catalog: Catalog): Readonly<Catalog> {
  ...
  for (const point of catalog.points) {
    if (seen.has(point.id)) throw new Error(`forge: duplicate injection point "${point.id}"`)
    seen.add(point.id)
  }
