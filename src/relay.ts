...
export interface ForgeRelayOptions {
  /** Exact webserver route, e.g. '/forge/snapshot'. */
  path: string
 ...
 */
export function createForgeRelay(options: ForgeRelayOptions) {
  if (typeof options.path !== 'string' || !options.path.startsWith('/')) {
    throw new Error(`forge-relay: path must start with '/', got ${JSON.stringify(options.path)}`)
  }
  if (!Array.isArray(options.points) || options.points.some((point) => typeof point !== 'string')) {
    throw new Error('forge-relay: points must be an array of point ids')
  }

  return {
    name: `forge-relay:${options.path}`,
    apply(ctx: Context) {
      const latest = new Map<string, ForgeEvent>()
      const listeners = options.points.map((point) => ctx.on(point as any, (event: ForgeEvent) => {
      ...
      const mount = (webServer: WebServerLike) => {
        ...
        const snapshot = (): ForgeSnapshot => ({ events: [...latest.values()] })
      ...
      ctx.effect(() => () => {
        ...
        routeDispose?.()
        routeDispose = undefined
      }, `forge-relay:cleanup(${options.path})`)
    },
  }
}
