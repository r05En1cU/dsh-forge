...
const unwrap = (value: any) => value?.[kOriginal] ?? value

function isOptedOut(proto: object): boolean {
  for (let c = (proto as any).constructor; c && c !== Function.prototype; c = Object.getPrototypeOf(c)) {
    if (Object.hasOwn(c, kOptOut) && c[kOptOut]) return true
  }
  return false
}
 ...
 */
export function createPrototypeBackend(): Backend {
  return {
    name: 'prototype',
    available: () => true,
    bind(ctx, point, hooks, options) {
      ...
      const patched = new Map<object, Entry>()
      let status: BindStatus = 'pending'

      const retire = (proto: object) => {
      ...
      const attach = (value: any): BindStatus => {
        const raw = unwrap(value)
        const proto = raw && Object.getPrototypeOf(raw)
        if (!proto || proto === Object.prototype) return 'missing'
        ...
        if (!patched.has(proto)) {
          for (const oldProto of [...patched.keys()]) retire(oldProto)
        }
        if (patched.has(proto)) return 'bound'
        if (isOptedOut(proto)) return 'opted-out'
        const desc = Object.getOwnPropertyDescriptor(proto, method)
        if (!desc || typeof desc.value !== 'function') return 'missing'
      ...
      return {
        ...
        verify: () => status,
        dispose: () => {
          for (const proto of [...patched.keys()]) retire(proto)
        },
      }
    },
  }
