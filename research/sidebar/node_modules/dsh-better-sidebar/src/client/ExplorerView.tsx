...
export function ExplorerView(props: {
  ...
  /** Present when the forge event stream should drive the root listing. */
  ctx?: Context
}) {
  const { sessionId, cwd, expanded, onToggle, onOpenFile, onReferenceFile, ctx } = props
  ...
  // Forge event stream (dsh-plugin-demo host relay): `sidebar/files` carries
  // the root listing. When present it wins over the API cache for the root
  // level only; deeper levels keep lazy-loading through the official API.
  useEffect(() => {
